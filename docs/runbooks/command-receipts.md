...
- 每次正式遊戲編輯命令會在鎖定自己的目標收據後，於同一交易內機會式清理最多 100 筆已完成且到期的其他收據；所有命令採相同鎖順序，批次以 `FOR UPDATE SKIP LOCKED` 跳過別人的目標鎖，避免交叉等待。`PostgresGameStore.cleanupExpiredCommandReceipts(limit)` 另提供最多 500 筆的維運批次，因此沒有外部 cron 仍會隨實際寫入持續收斂，也可由單一排程重複執行。
- 到達 `expires_at` 後，同一命令會在取得自己的收據鎖後，以 `clock_timestamp()` 重新判定並逐筆排除舊收據，不依賴交易開始時間或 100 筆批次是否剛好清到它；接著依目前資源版本重新判定，通常回傳具名版本衝突。其餘實體刪除採有界、最終一致的清理，不承諾在第 90 天整點同步移除所有列。
