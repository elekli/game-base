...
- 每次正式遊戲編輯命令會先在同一交易內機會式清理最多 100 筆已完成且到期的收據；`PostgresGameStore.cleanupExpiredCommandReceipts(limit)` 另提供最多 500 筆的維運批次。兩者都使用 `FOR UPDATE SKIP LOCKED`，因此沒有外部 cron 仍會隨實際寫入持續收斂，也可由單一排程重複執行。
- 到達 `expires_at` 後，同一命令會先逐筆排除自己的舊收據，不依賴 100 筆批次是否剛好清到它；接著依目前資源版本重新判定，通常回傳具名版本衝突。其餘實體刪除採有界、最終一致的清理，不承諾在第 90 天整點同步移除所有列。
