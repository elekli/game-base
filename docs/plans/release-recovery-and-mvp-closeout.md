...
- Vercel Production 的 `SUPABASE_SECRET_KEY` 與 `EXPECTED_SUPABASE_SECRET_KEY_SHA256` 已更新到 active key；repository Production binding 與測試同步新指紋。合併前必須以 `pnpm release:settings:check` 同時核對 Supabase active key、Vercel 變數與 repository binding，再走完整 CI 與受保護發布。
