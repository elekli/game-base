...
Canary contract v2 為每次完整嘗試建立一次 UUIDv4 generation，並要求資料列、Storage 物件與證據都帶同一代別；每個動作另帶單調遞增的 action sequence，回傳事件必須精確匹配目前動作，避免同一 generation 內的晚到事件被重播。只有同 generation 的 `1/0 row_claimed` 或 `1/1 object_written` 殘留可自動清理；`0/1`、不同 generation／identity／payload hash，或 Storage／cleanup 結果不確定時一律停止並要求人工復原。目前這仍是純狀態模型：尚未加入持久層、資料庫／Storage adapter 或可執行的 live runner，因此不改變 fail-closed 發布閘。
