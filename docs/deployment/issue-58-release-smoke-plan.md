...
本切片只建立 production release smoke 所需的專用身分、內部 route、固定資料列與固定 Storage 物件操作。它不啟用 Vercel live mutation、不建立 production workflow，也不宣稱已驗證 direct-origin denial、custom-domain routing 或 logical restore。這些外部前提未齊時，既有 `productionDeploymentEnabled=false` 與 smoke runner 的 fail-closed barrier 維持不變。
...
| S4 | Safety | route 的 caller-controlled input 只有完整 40 字元小寫 commit SHA、由 runner 每次完整 smoke attempt 產生並在該 attempt 重用的 UUIDv4 fencing generation，以及封閉操作 discriminant；request body 最大 1 KiB，拒絕額外欄位。table、row ID、object path、owner、payload、URL 與 count 上限不能由 caller 指定。generation 不是授權憑證，只是跨 request 的新舊排序 token。 |
...
`run-fixed-read-checks` 的 application read target 固定為 production binding 已驗證的單一 owner library：route 不接受 owner input，而是從既有 `config.cloudflare.ownerSub` 建立內部 `OwnerIdentity`，呼叫新增的 `probeLibraryReadable(owner)` 最小介面。Postgres 實作只做固定 `SELECT EXISTS (... LIMIT 1)`，空收藏庫仍算成功；成功判準是有界查詢完成，response 只回 `authenticated-library-read: passed`，不回 exists、count、名稱、ID 或任何 game payload。這是專用固定 probe，不呼叫全量 `listLibraryGames()`，不把 service principal 傳給 `requireOwner`，也不暴露一般 private route 能力。
...
- 每個 adapter operation 都用 `pg_try_advisory_xact_lock` 取得同一固定 lock，不排隊等待；DB statement／transaction 各有固定 deadline。row write 經受控函式採 generation＋expected-phase conditional transition；不同 generation、identity 或 hash 具名拒絕。
...
| `1/0`＋相同 generation／identity／hash＋pre-Storage 確定 phase | 視為同 attempt partial residue，經受控函式清除後重跑 | production-smoke contract 升為 v2：`residuePolicy` 明載只允許此 partial case 與相同 generation 的完整 pair；state model 新增 generation、phase、partial-cleanup transition／event evidence 與 tests。 |
...
5. 接上由 `ownerSub` binding 導出的固定 `LIMIT 1` library probe 與 runtime DB read，測空／非空 library 都只回 pass；匿名 private Storage denial與外部網域檢查仍留給 runner。
...
- 上述條件到位且 route／migration 完成獨立審查前，不啟用 live deployment transport 或 smoke runner。
