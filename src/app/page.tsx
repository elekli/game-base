...
export default async function HomePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [games, sourceCategories, contributorFacets, platforms, tags] = await Promise.all([libraryService.listGames(filters), libraryService.listSourceCategoryFacets(filters.media ?? []), libraryService.listContributorFacets(), libraryService.listPlatforms(), libraryService.listTags()]);
  return <main className="mx-auto min-h-screen max-w-3xl px-4 py-8 sm:px-6"><header className="mb-8 flex items-end justify-between gap-4"><div><p className="text-xs font-semibold tracking-[0.2em] text-emerald-800">PUIZERU GAMEBASE</p><h1 className="mt-2 text-3xl font-semibold">我的收藏庫</h1></div><Link className="rounded-full bg-emerald-900 px-4 py-2 text-sm font-semibold text-white" href="/games/new">新增遊戲</Link></header><LibraryClient games={games} sourceCategories={sourceCategories} contributorFacets={contributorFacets} filters={filters} sharedPlatforms={platforms} sharedTags={tags} /></main>;
}
