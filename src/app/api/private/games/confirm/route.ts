...
export async function POST(request: Request) {
  return handlePrivateRequest(request, { ...getPrivateDependencies(), operation: async () => {
    let body: { provider?: unknown; sourceId?: unknown };
    try { body = await request.json(); } catch { throw new PrivateRequestInputError("請求格式無效。"); }
    if ((body.provider !== "bgg" && body.provider !== "igdb") || typeof body.sourceId !== "string") throw new PrivateRequestInputError("來源參數無效。");
    ...
    return gamesService.getExternalGameConfirmation({ ref });
  } });
}
