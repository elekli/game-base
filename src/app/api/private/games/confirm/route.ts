...
export async function POST(request: Request) {
  const body = await request.json() as { provider?: "bgg" | "igdb"; sourceId?: string };
  if (body.provider === undefined || body.sourceId === undefined) return Response.json({ message: "來源參數無效。" }, { status: 400 });
  ...
  return handlePrivateRequest(request, { ...getPrivateDependencies(), operation: async () => gamesService.getExternalGameConfirmation({ ref }) });
}
