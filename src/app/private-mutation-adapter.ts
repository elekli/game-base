import { z } from "zod";
import { assertReference, type GamesService } from "@/modules/games";
...
const addContributionSchema = z.object({ gameId: z.uuid(), name: z.string().trim().min(1).max(200), entityKind: z.enum(["person", "company"]), role: z.enum(["design", "art", "publisher"]) });
...
export type PrivateMutationAdapter = Readonly<{
  addManualContribution(input: unknown): Promise<PrivateActionResult<{ possibleDuplicate: boolean }>>;
  ...
  return {
    addManualContribution(input) {
      return boundary({ input, schema: addContributionSchema, inputErrorMessage: "貢獻關係參數無效。", operation: async (parsed) => {
        const { possibleDuplicate } = await libraryService.addManualContribution(parsed);
