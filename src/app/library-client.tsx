...
type Props = Readonly<{ games: readonly GameRecord[]; facets: readonly GameRecord[]; filters: LibraryFilters }>;
...
export function LibraryClient({ games, facets, filters }: Props) {
  ...
  const contributors = [...new Map(facets.flatMap((game) => game.contributors.map((contributor) => [contributor.id, contributor] as const))).values()].sort((a, b) => a.name.localeCompare(b.name, "zh-Hant"));
  ...
  return <>
    <form className="mb-6 space-y-3 rounded-2xl border border-slate-200 bg-white p-4" role="search">
      ...
      {contributors.length > 0 && <fieldset><legend className="mb-2 text-sm font-medium">貢獻者</legend><div className="flex flex-wrap gap-3">{contributors.map((contributor) => <label className="flex items-center gap-2 text-sm" key={contributor.id}><input type="checkbox" name="contributor" value={contributor.id} defaultChecked={(filters.contributors ?? []).includes(contributor.id)} />{contributor.name}</label>)}</div></fieldset>}
