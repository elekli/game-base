    ...
    <details>
      ...
      <div className="mt-4 space-y-4">
        <div>
          ...
          {sourceContributions.length === 0 ? <p className="mt-1 text-sm text-slate-500">沒有來源貢獻。</p> : <ul className="mt-1 space-y-1 text-sm">{sourceContributions.map((item) => <li key={`${item.id}:${item.role}`}>{item.contributorId ? <Link className="font-medium text-emerald-900 underline underline-offset-2" href={`/?contributor-${item.role}=${encodeURIComponent(item.contributorId)}`} aria-label={`查看 ${item.name} 的收藏庫遊戲`}>{item.name}</Link> : <span>{item.name}</span>} · {roleLabels[item.role]} · {item.provider?.toUpperCase()}</li>)}</ul>}
        </div>
        <div>
          <h3 className="text-sm font-medium">手動貢獻</h3>
          {manualContributions.length > 0 && <ul className="mt-1 space-y-1 text-sm">{manualContributions.map((item) => <li className="flex items-center justify-between gap-2" key={item.id}><span>{item.contributorId ? <Link className="font-medium text-emerald-900 underline underline-offset-2" href={`/?contributor-${item.role}=${encodeURIComponent(item.contributorId)}`} aria-label={`查看 ${item.name} 的收藏庫遊戲`}>{item.name}</Link> : item.name} · {roleLabels[item.role]}</span><button type="button" className="text-sm text-rose-700" onClick={() => void removeManual(item.id)}>移除</button></li>)}</ul>}
