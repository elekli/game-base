...
- [定稿 MVP 分批實作與驗證計畫](https://github.com/elekli/game-base/issues/11) 已認領並完成本機規劃主文件：`docs/plans/mvp-implementation-plan.md`。它切成 0～5 六批，首個可用縱切包含 BGG／IGDB 雙來源搜尋、列內確認與直接入庫；並附帶 gap-verification 的不變式、交錯與權限路徑檢查。
- 此票尚待 push、PR、resolution comment、主地圖索引與關閉。個人 SSH key 尚未載入 agent，因此本 session 的 `git fetch` 被 `Permission denied (publickey)` 阻擋；不要重複嘗試，先依下方指令由 elek 載入 key。
...
1. 由 elek 在自己的終端執行 `SSH_AUTH_SOCK=~/.ssh/agent.sock ssh-add ~/.ssh/id_personal`；agent 再以相同 `SSH_AUTH_SOCK` 推送 `docs/11-mvp-implementation-plan`、開 PR、完成 review 與合併。
2. 回到 [定稿 MVP 分批實作與驗證計畫](https://github.com/elekli/game-base/issues/11) 貼 resolution comment、關閉票券，並在 [規劃可執行的 MVP 實作路線](https://github.com/elekli/game-base/issues/1) 的 Decisions so far 追加一行連結摘要。
